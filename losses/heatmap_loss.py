...
from common.root_ops import encode_box_relative_root, make_gaussian_heatmap


class HeatmapRootLoss(v8SegmentationLoss):
    """
    Multi-task loss for ROI / Instance Heatmap Root Localization:
        box + segmentation + classification + DFL box loss + 2D Gaussian MSE Heatmap loss
    """

    def __init__(self, model, class_weights=None, heatmap_size=16, heatmap_sigma=1.5, loss_type="mse"):
    ...
    def __call__(self, preds, batch):
        ...
        # Raw Heatmap logits: shape (bs, H * W, n_anchors)
        pred_kpts_heatmap = preds["kpts_heatmap"].permute(0, 2, 1).contiguous()  # (bs, n_anchors, H * W)
        pred_kpts = preds["kpts"].permute(0, 2, 1).contiguous()                  # (bs, n_anchors, 2)
        ...
        if fg_mask.sum():
            ...
            loss[4] = self._heatmap_root_loss(
                fg_mask,
                target_gt_idx,
                gt_kpts,
                target_bboxes,
    ...
    def _heatmap_root_loss(
        ...
        for i, (fg, gt_idx) in enumerate(zip(fg_mask, target_gt_idx)):
            ...
            matched_heatmap_logits = pred_kpts_heatmap[i][fg].view(-1, H, W)  # (M, H, W)

            # Target local heatmap coordinates
            heat_x = matched_gt_uv[:, 0] * (W - 1)
            heat_y = matched_gt_uv[:, 1] * (H - 1)

            # Target 2D Gaussian heatmap
            target_heatmap = make_gaussian_heatmap(
                heat_x, heat_y, H, sigma=self.heatmap_sigma, device=self.device
            )
