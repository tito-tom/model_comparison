...
class RootDFLLoss(v8SegmentationLoss):
    """
    Multi-task loss for Box-Relative DFL Root Regression:
        box + segmentation + classification + DFL box loss + Distribution Focal Loss for root
    """
    ...
    def __call__(self, preds, batch):
        ...
        # Raw DFL logits for root: shape (bs, 2 * root_bins, n_anchors)
        pred_kpts_logits = preds["kpts_logits"].permute(0, 2, 1).contiguous()  # (bs, n_anchors, 2 * B)
        ...
        if fg_mask.sum():
            ...
            loss[4], self.last_entropy = self._root_dfl_loss(
                fg_mask,
                target_gt_idx,
                gt_kpts,
                target_bboxes,
    ...
    def _root_dfl_loss(
        ...
        for i, (fg, gt_idx) in enumerate(zip(fg_mask, target_gt_idx)):
            ...
            # Encode GT root point to box-relative (u_gt, v_gt) in [0, 1]
            matched_gt_uv = encode_box_relative_root(
                matched_gt_pixel, matched_gt_box_pixel
            )

            matched_logits = pred_kpts_logits[i][fg]  # (M, 2 * B)
            matched_pred_uv = pred_kpts_uv[i][fg]     # (M, 2)

            logits_u = matched_logits[:, :B]
            logits_v = matched_logits[:, B:]

            gt_u = matched_gt_uv[:, 0]
            gt_v = matched_gt_uv[:, 1]

            # DFL target interpolation for u and v
            left_u, right_u, wl_u, wr_u = encode_dfl_target(gt_u, B)
            left_v, right_v, wl_v, wr_v = encode_dfl_target(gt_v, B)

            loss_u = F.cross_entropy(logits_u, left_u, reduction="none") * wl_u + \
                     F.cross_entropy(logits_u, right_u, reduction="none") * wr_u

            loss_v = F.cross_entropy(logits_v, left_v, reduction="none") * wl_v + \
                     F.cross_entropy(logits_v, right_v, reduction="none") * wr_v

            dfl_loss = (loss_u + loss_v) / 2.0

            # Auxiliary Smooth-L1 loss on expected (u_hat, v_hat)
            aux_l1 = F.smooth_l1_loss(matched_pred_uv, matched_gt_uv, reduction="none").sum(-1)
            ...
            entropy_u_list.append(distribution_entropy(probs_u).mean().item())
            entropy_v_list.append(distribution_entropy(probs_v).mean().item())

        mean_eu = float(torch.tensor(entropy_u_list).mean().item()) if entropy_u_list else 0.0
        mean_ev = float(torch.tensor(entropy_v_list).mean().item()) if entropy_v_list else 0.0
        mean_root_e = (mean_eu + mean_ev) / 2.0

        entropy_dict = {
            "mean_entropy_u": mean_eu,
            "mean_entropy_v": mean_ev,
